<footer class="pt-4">
    <div class="container-fluid p-4">
      <div class="row">
        <div class="col-md-6 mb-4">
          <h3 class="titles-footer">¿Deseas contactarte con nosotros?</h3>
          <br>
          <ul class="ul-footer ul-link-nav flex-wrap d-flex flex-row justify-content-start">
            <li>
              <a href="tel: +573222325596" ;>Número telefónico: <br> 322 232 5596 </a>
            </li>
            <li>
              <a
                href="mailto:estebanaluisv@juandelcorral.edu.co?Subject=Interesad@%20en%20hacer%20reservacion"
                >Correo Electronico: <br> DreamScape@gmail.com
              </a>
            </li>
          </ul>
        </div>
        
        <div class="col-md-6 mb-4">
        <h2 class="titles-footer text-md-end" >Redes Sociales</h2>
        <div class= "d-flex flex-row justify-content-start justify-content-md-end">
        <a
              href="https://wa.me/+573173077666?text=Hola,%20estoy%20interesad@%20en%20hacer%20una%20reservación"
              target="_blank"
              >
              <img
                width="55px"
                src="https://img.icons8.com/color/48/000000/whatsapp--v4.png"
              />
            </a>
            <a href="">
              <img
                width="55px"
                src="https://img.icons8.com/fluency/48/000000/facebook-circled.png"
              />
            </a>
            <a href="">
              <img
                width="55px"
                src="https://img.icons8.com/fluency/48/000000/instagram-new.png"
              />
            </a>
            </div>
        </div>
      </div>
      <hr>
      <div class="row">
         <div class="col-md-4">
                <img width="23%" src="{{asset("img/palmera.png")}}" />
          </div>
          <div class="col-md-3 offset-md-5">
          <p> © 2023 DreamScape Resort. All Rights reserved. <br>
          Powered by: no one </p>
          </div>
      </div>        
    </div> 
  </footer>
  
  
              